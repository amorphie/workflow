...
public record PostStateDefinitionRequest(string name, MultilanguageText title, StatusType baseStatus, 
StateType type,
 PostTransitionDefinitionRequest[] transitions, 
 bool? ispublicForm, 
 UiFormDto[]? publicForms, 
 UiFormDto[]? uiForms, 
 MFATypeEnum? mfaType,
 string? subWorkflowName, 
 string? initPageName,
 string[]? allowedSuffix);
...
// bool? ispublicForm, UiFormDto[]? publicForms, MFATypeEnum? mfaType, string? subWorkflowName, string? initPageName,string[]? allowedSuffix) :
//     PostStateDefinitionRequest(name, title, baseStatus, StateType.Start, transitions, ispublicForm, publicForms, mfaType, subWorkflowName, initPageName,allowedSuffix);
...
//  bool? ispublicForm, UiFormDto[]? publicForms, MFATypeEnum? mfaType, string? subWorkflowName, string? initPageName,string[]? allowedSuffix) :
//     PostStateDefinitionRequest(name, title, baseStatus, StateType.Finish, transitions, ispublicForm, publicForms, mfaType, subWorkflowName, initPageName,allowedSuffix);
...
//  long ExecuteInMinutes, bool? ispublicForm, UiFormDto[]? publicForms, MFATypeEnum? mfaType, string? subWorkflowName, string? initPageName,string[]? allowedSuffix) :
//     PostStateDefinitionRequest(name, title, baseStatus, StateType.Standart, transitions, ispublicForm, publicForms, mfaType, subWorkflowName, initPageName,allowedSuffix);


public record PostTransitionDefinitionRequest(string name, MultilanguageText title, string toState, UiFormDto[]? form, UiFormDto[]? uiForms, string fromState, string? serviceName, string? message, string? gateway, PostPageDefinitionRequest? page, MultilanguageText[]? historyForms, TypeofUiEnum? typeofUi, TransitionButtonType? buttonType);
