...
public partial class StateService : IStateService
{
    ...
    public async Task<Response> SaveBulkAsync(WorkflowCreateDto data, string workflowName, string language)
    {
        ...
        {
            var stateSaveResponse = await SaveAsync(item, workflowName, language);
        }
    ...
    public async Task<Response> SaveAsync(StateCreateDto data, string workflowName, string language)
    {
    ...
    private void SaveTitle(State state, List<core.Dtos.MultilanguageText> titles)
    {
        foreach (var languageForm in titles)
        {
            ...
            {
                var newTranslation = InsertTranslation(languageForm);
                state.Titles.Add(newTranslation);
            }
        }
    }
    ...
    private void SavePublicForm(State state, List<core.Dtos.MultilanguageText> publicForms)
    {
        foreach (var languageForm in publicForms)
        {
            ...
            {
                var newTranslation = InsertTranslation(languageForm);
                state.PublicForms.Add(newTranslation);
            }
        }
    }
    private void SaveForm(Transition transition, List<core.Dtos.MultilanguageText> forms)
    {
        foreach (var languageForm in forms)
        {
            ...
            {
                var newTranslation = InsertTranslation(languageForm);
                transition.Forms.Add(newTranslation);
            }
        }
    }
            ...
            else
            {
                if (languageForm.forms != null && languageForm.forms.Any())
                {
                    foreach (var languagePF in languageForm.forms)
                    {
                        ...
                        if (translation == null)
                        {
                            var newTranslation = InsertTranslation(languagePF);
                            uiForm.Forms.Add(newTranslation);
    ...
    private Translation InsertTranslation(core.Dtos.MultilanguageText language)
    {
